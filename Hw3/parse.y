...
%%
program:        MODULE var_ID {genProgramStart($2->id);} program_decl BEG {genMainStart();} statement END {genMainEnd();} var_ID {
			if(strcmp($2->id,$10->id) != 0) yyerror("Module ID No Error");
...
simple:		var_ID SET expression';'{
			Trace("ID := exp");
			if(lookup($1->id) == 0) yyerror("ID No define1");
		...
		|var_ID'['expression']'SET expression';'{
			Trace("ID[exp] := EXP");
			if(lookup($1->id) == 0) yyerror("ID No define2");
			else $1 = ReturnID($1->id);
		...
		|READ var_ID';'	{Trace("Read ID"); if(lookup($2->id) == 0) yyerror("ID No define3"); }
		|PRINT {genPrintStart();} expression';' {
			Trace("PRINT");
			genPrint($3->type);		
		}
		|PRINTLN {genPrintStart();} expression';' {
			Trace("PRINTLN");
			genPrintln($3->type);
		}
		|RETURN';'{
...
expression:	var_ID	{
			Trace("ID to exp");
			if(lookup($1->id) == 0) yyerror("ID No define4");
			else $1 = ReturnID($1->id);
		...
		|var_ID'['expression']'{
			Trace("Array to exp");
			if(lookup($1->id) == 0) yyerror("ID No define5");
			else $1 = ReturnID($1->id);
			if($1->type != 4) yyerror("Not Array Type");
			else if($3->type != 0) yyerror("index not int");
			if($1->arr1 > $3->ival || $3->ival > $1->arr2) yyerror("Array Out Of Range");
...
fun:		fun fun { Trace("Function combine")}
		|PROCEDURE var_ID fun_var BEG {genFuncStart($2,$3->returnType,FuncDelcNum(lead)); SetLocalIndex();} statement END {genFuncEnd();} var_ID ';' {
			Trace("Proceduce Delcare");
			if(strcmp($2->id,$9->id) != 0) yyerror("Proceduce ID error");
			insertLocalToFunc($3);
			if(CompareFuncReturnType($3) == 0) yyerror("Return type error");
			if(lookup($2->id) == 0) insertFun($3,$2->id);
			else yyerror("Func ID repeat");
		}
		|PROCEDURE var_ID fun_var var_delc_loc BEG {genFuncStart($2,$3->returnType,FuncDelcNum(lead)); SetLocalIndex();} statement END {genFuncEnd();} var_ID ';' {
			Trace("Proceduce Delcare");
			if(strcmp($2->id,$10->id) != 0) yyerror("Proceduce ID error");
			insertLocalToFunc($3);
			if(CompareFuncReturnType($3) == 0) yyerror("Return type error");
			if(lookup($2->id) == 0) insertFun($3,$2->id);
...
loop:		WHILE {genWhileStart();} '(' expression ')' {genWhileCond();} DO statement END {genWhileEnd();}';'{
			Trace("Loop");
			if($4->type != 2) yyerror("loop's type error");
		};

condition:	condIF THEN statement END {genIfEnd();} ';'{
			Trace("Condition IF");
		}
		| condIF  THEN  statement ELSE {genElse();} statement END {genIfElseEnd();}';'{
			Trace("condition IF ELSE");	
		};

condIF:		IF '(' expression ')' { 
...
const_loc:	const_loc const_loc {Trace("Const const combine")}
		|var_com_loc ':' var_type '[' INT_CON ',' INT_CON ']' OF var_type';'{
		...
		|var_ID '=' con_Var ';'{
			...
			if(lookupLocal($1->id) == 0) insertConst($3,$1->id,1);
			else yyerror("ID repeat");
...
var_com_loc:	var_ID{
			Trace("var commman begin");
			if(lookupLocal($1->id) == 0) insertVar($1,1);
			else yyerror("ID repeat");
		}
		|var_com_loc ',' var_ID{
			Trace("var commman , var");
			if(lookupLocal($3->id) == 0) insertVar($3,1);
			else yyerror("ID repeat");		
		};
